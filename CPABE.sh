...
##generate key
#{ time ./oabe_keygen -s CP  -i "($str01)"  -o studentCP$i 2> sleep.stderr ; } 2>> generatekeytime.txt
...
#        echo $str01 >> abeencrypttime.txt
##encryption
#	echo encryption >> abeencrypttime.txt
#{ time ./oabe_enc -s CP -e "$str01"  -i file.zip -o file$i.cpabe 2>> sleep.stderr ; } 2>> abeencrypttime.txt

##generate key
#	echo generate key >> abeencrypttime.txt
#        { time ./oabe_keygen -s CP  -i "$str01"  -o studentCP$i 2>> sleep.stderr ; } 2>> abeencrypttime.txt


##decryption
#	echo decryption >> abeencrypttime.txt
#        { time ./oabe_dec -s CP -k studentCP$i.key -i file$i.cpabe -o file_dec$i.zip  2>> sleep.stderr ; } 2>> abeencrypttime.txt
#add attribute
#	str01=${str01}" or "${i}
